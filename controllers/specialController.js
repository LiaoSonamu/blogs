...
module.exports = {
  ...
  register(req, res) {
    ...
    delete req.session.registerCode; // 验证通过后清除session
    new UserModel({nickname: email, email: email, password: crypto.createHash('md5').update(password).digest('hex')}).save((e, r) => {
      if(e) res.json({code: -1, message: '注册失败，请重试'});
      delete r.password; // session存入用户信息，删除密码字段
